...
{
    tuuvm_identitySet_t *result = (tuuvm_identitySet_t*)tuuvm_context_allocatePointerTuple(context, context->roots.setType, TUUVM_SLOT_COUNT_FOR_STRUCTURE_TYPE(tuuvm_identitySet_t));
...
TUUVM_API bool tuuvm_identitySet_findWithExplicitHash(tuuvm_tuple_t set, void *element, tuuvm_identitySet_explicitHashFunction_t hashFunction, tuuvm_identitySet_explicitEqualsFunction_t equalsFunction, tuuvm_tuple_t *outFoundElement)
{
    *outFoundElement = TUUVM_NULL_TUPLE;
    ...
    tuuvm_identitySet_t *setObject = (tuuvm_identitySet_t*)set;
...
static intptr_t tuuvm_identitySet_scanFor(tuuvm_tuple_t set, tuuvm_tuple_t element)
{
    if(!tuuvm_tuple_isNonNullPointer(set))
        return -1;
    ...
    {
        tuuvm_tuple_t setElement = storage->elements[i];
        if(setElement == TUUVM_HASHTABLE_EMPTY_ELEMENT_TUPLE || tuuvm_tuple_identityEquals(element, setElement))
            return (intptr_t)i;
    }
...
TUUVM_API bool tuuvm_identitySet_find(tuuvm_tuple_t set, tuuvm_tuple_t element, tuuvm_tuple_t *outFoundElement)
{
    *outFoundElement = TUUVM_NULL_TUPLE;
    ...
    intptr_t elementIndex = tuuvm_identitySet_scanFor(set, element);
    if(elementIndex < 0)
        return false;
...
static void tuuvm_identitySet_insertNoCheck(tuuvm_tuple_t set, tuuvm_tuple_t element)
{
    intptr_t elementIndex = tuuvm_identitySet_scanFor(set, element);
...
static void tuuvm_identitySet_increaseCapacity(tuuvm_context_t *context, tuuvm_tuple_t set)
{
    tuuvm_identitySet_t *setObject = (tuuvm_identitySet_t*)set;
    ...
    for(size_t i = 0; i < oldCapacity; ++i)
    {
        ...
        if(element != TUUVM_HASHTABLE_EMPTY_ELEMENT_TUPLE)
            tuuvm_identitySet_insertNoCheck(set, element);
    }
}
...
TUUVM_API void tuuvm_identitySet_insert(tuuvm_context_t *context, tuuvm_tuple_t set, tuuvm_tuple_t element)
{
    ...
    if(elementIndex < 0)
    {
        tuuvm_identitySet_increaseCapacity(context, set);
        elementIndex = tuuvm_identitySet_scanFor(set, element);
        if(elementIndex < 0)
