...
static void sysbvm_orderedCollection_increaseCapacityToAtLeast(sysbvm_context_t *context, sysbvm_tuple_t stringStream, size_t requiredCapacity)
{
...
SYSBVM_API void sysbvm_stringStream_nextPut(sysbvm_context_t *context, sysbvm_tuple_t stringStream, uint8_t character)
{
    ...
    if(requiredCapacity > capacity)
        sysbvm_orderedCollection_increaseCapacityToAtLeast(context, stringStream, requiredCapacity);
...
void sysbvm_stringStream_setupPrimitives(sysbvm_context_t *context)
{
    ...
    sysbvm_context_setIntrinsicSymbolBindingValueWithPrimitiveMethod(context, "StringStream::nextPutAllBytesOf:", context->roots.stringStreamType, "nextPutAllBytesOf:", 2, SYSBVM_FUNCTION_FLAGS_CORE_PRIMITIVE, NULL, sysbvm_stringStream_primitive_nextPutAll);
